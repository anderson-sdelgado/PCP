...
class RecoverTerceiroServerImplTest : KoinTest {
    ...
    fun verify_return_failure_usecase_if_not_have_data_config_internal() = runTest {
        ...
        server.start()
        server.enqueue(
            MockResponse().setBody(resultTerceiro)
        )
        loadKoinModules(
    ...
    fun verify_return_failure_datasource_if_access_invalid_page() = runTest {
        ...
        assertEquals(
            result.exceptionOrNull()!!.message,
            "IGetServerTerceiro -> ITerceiroRepository.recoverAll -> ITerceiroRetrofitDatasource.recoverAll"
        )
        assertEquals(
            result.exceptionOrNull()!!.cause.toString(),
            "java.lang.NullPointerException"
    ...
    fun verify_return_failure_datasource_if_token_invalid() = runTest {
        ...
        server.start()
        server.enqueue(
            MockResponse().setBody(tokenInvalidTerceiro)
        )
        loadKoinModules(
            generateTestAppComponent(
                server.url("/").toString()
    ...
    fun verify_return_failure_repository_if_data_incorrect() = runTest {
        ...
        server.start()
        server.enqueue(
            MockResponse().setBody(resultTerceiroIncorrect)
        )
        loadKoinModules(
            generateTestAppComponent(
                server.url("/").toString()
        ...
        assertEquals(
            result.exceptionOrNull()!!.message,
            "IGetServerTerceiro -> ITerceiroRepository.recoverAll"
        )
        assertEquals(
            result.exceptionOrNull()!!.cause.toString(),
            "java.lang.NullPointerException: Parameter specified as non-null is null: method br.com.usinasantafe.pcp.domain.entities.stable.Terceiro.<init>, parameter cpfTerceiro"
        )
    }
    ...
    fun verify_return_data_if_success_usecase() = runTest {
        ...
        assertEquals(
            resultTerceiro.idTerceiro,
            1
        )
        assertEquals(
            resultTerceiro.idBDTerceiro,
            1
        )
        assertEquals(
            resultTerceiro.nomeTerceiro,
            "Terceiro"
        )
        assertEquals(
            resultTerceiro.cpfTerceiro,
            "123.456.789-00"
        )
        assertEquals(
            resultTerceiro.empresaTerceiro,
            "Empresa Terceiro"
        )
    }
