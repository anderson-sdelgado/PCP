...
class RecoverLocalServerImplTest : KoinTest {
    ...
    fun verify_return_failure_usecase_if_not_have_data_config_internal() = runTest {
        ...
        server.start()
        server.enqueue(
            MockResponse().setBody(resultLocal)
        )
        loadKoinModules(
    ...
    fun verify_return_failure_datasource_if_access_invalid_page() = runTest {
        ...
        assertEquals(
            result.exceptionOrNull()!!.message,
            "IGetServerLocal -> ILocalRepository.recoverAll -> ILocalRetrofitDatasource.recoverAll"
        )
        assertEquals(
            result.exceptionOrNull()!!.cause.toString(),
            "java.lang.NullPointerException"
    ...
    fun verify_return_failure_datasource_if_token_invalid() = runTest {
        ...
        server.start()
        server.enqueue(
            MockResponse().setBody(tokenInvalidLocal)
        )
        loadKoinModules(
            generateTestAppComponent(
                server.url("/").toString()
    ...
    fun verify_return_failure_repository_if_data_incorrect() = runTest {
        ...
        server.start()
        server.enqueue(
            MockResponse().setBody(resultLocalIncorrect)
        )
        loadKoinModules(
            generateTestAppComponent(
                server.url("/").toString()
        ...
        assertEquals(
            result.exceptionOrNull()!!.message,
            "IGetServerLocal -> ILocalRepository.recoverAll"
        )
        assertEquals(
            result.exceptionOrNull()!!.cause.toString(),
            "java.lang.NullPointerException: Parameter specified as non-null is null: method br.com.usinasantafe.pcp.domain.entities.stable.Local.<init>, parameter descrLocal"
        )
    }
