...
class ISaveMovEquipVisitTercTest: KoinTest {
    ...
    fun check_return_failure_if_not_have_data_output() =
        runTest {
            ...
            assertEquals(
                ...
                "ISaveMovEquipVisitTerc -> IMovEquipVisitTercRepository.setOutside -> IMovEquipVisitTercRoomDatasource.setOutside"
            )
            assertEquals(
                result.exceptionOrNull()!!.cause.toString(),
                "java.lang.NullPointerException: Attempt to invoke virtual method 'void br.com.usinasantafe.pcp.infra.models.room.variable.MovEquipVisitTercRoomModel.setStatusMovEquipForeigVisitTerc(br.com.usinasantafe.pcp.utils.StatusForeigner)' on a null object reference"
            )
        }
    ...
    fun check_return_failure_if_have_data_mov_config_and_without_mov_shared_preferences_and_output() =
        runTest {
            ...
            assertEquals(
                result.exceptionOrNull()!!.message,
                "ISaveMovEquipVisitTerc -> IMovEquipVisitTercRepository.save -> IMovEquipVisitTercSharedPreferencesDatasource.get"
            )
            assertEquals(
                result.exceptionOrNull()!!.cause.toString(),
                "java.lang.NullPointerException"
    ...
    fun check_return_true_if_save_execute_successfully_and_output() =
        runTest {
            ...
            assertEquals(
                roomModel1.idMovEquipVisitTerc,
                1
            )
            assertEquals(
                roomModel1.tipoMovEquipVisitTerc,
                TypeMovEquip.INPUT
            )
            assertEquals(
                roomModel1.observMovEquipVisitTerc,
                "OBSERV TESTE"
            )
            assertEquals(
                roomModel1.statusMovEquipForeigVisitTerc,
                StatusForeigner.OUTSIDE
            )
            val roomModel2 = movEquipVisitTercDao.get(2)
            assertEquals(
                roomModel2.idMovEquipVisitTerc,
                2
            )
            assertEquals(
                roomModel2.tipoMovEquipVisitTerc,
                TypeMovEquip.OUTPUT
            )
            assertEquals(
                roomModel2.observMovEquipVisitTerc,
                null
            )
            assertEquals(
                roomModel2.statusMovEquipForeigVisitTerc,
                StatusForeigner.OUTSIDE
            )
        }
