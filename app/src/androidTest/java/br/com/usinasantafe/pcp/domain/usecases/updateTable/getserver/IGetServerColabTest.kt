...
class RecoverColabServerImplTest : KoinTest {
    ...
    fun verify_return_failure_datasource_if_access_invalid_page() = runTest {
        ...
        assertEquals(
            result.exceptionOrNull()!!.message,
            "IGetServerColab -> IColabRepository.recoverAll -> IColabRetrofitDatasource.recoverAll"
        )
        assertEquals(
            result.exceptionOrNull()!!.cause.toString(),
            "java.lang.NullPointerException"
    ...
    fun verify_return_failure_datasource_if_token_invalid() = runTest {
        ...
        server.start()
        server.enqueue(
            MockResponse().setBody(tokenInvalidColab)
        )
        loadKoinModules(
            generateTestAppComponent(
                server.url("/").toString()
    ...
    fun verify_return_failure_repository_if_data_incorrect() = runTest {
        ...
        server.start()
        server.enqueue(
            MockResponse().setBody(resultColabIncorrect)
        )
        loadKoinModules(
            generateTestAppComponent(
                server.url("/").toString()
        ...
        assertEquals(
            result.exceptionOrNull()!!.message,
            "IGetServerColab -> IColabRepository.recoverAll"
        )
        assertEquals(
            result.exceptionOrNull()!!.cause.toString(),
            "java.lang.NullPointerException: Parameter specified as non-null is null: method br.com.usinasantafe.pcp.domain.entities.stable.Colab.<init>, parameter nomeColab"
        )
    }
    ...
    fun verify_return_data_if_success_usecase() = runTest {
        ...
        assertEquals(
            resultColab.matricColab,
            19759
        )
        assertEquals(
            resultColab.nomeColab,
