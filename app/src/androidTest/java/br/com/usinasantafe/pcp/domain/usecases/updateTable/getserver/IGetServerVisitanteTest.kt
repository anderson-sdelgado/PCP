...
class RecoverVisitanteServerImplTest : KoinTest {
    ...
    fun verify_return_failure_usecase_if_not_have_data_config_internal() = runTest {
        ...
        server.start()
        server.enqueue(
            MockResponse().setBody(resultVisitante)
        )
        loadKoinModules(
    ...
    fun verify_return_failure_datasource_if_access_invalid_page() = runTest {
        ...
        assertEquals(
            result.exceptionOrNull()!!.message,
            "IGetServerVisitante -> IVisitanteRepository.recoverAll -> IVisitanteRetrofitDatasource.recoverAll"
        )
        assertEquals(
            result.exceptionOrNull()!!.cause.toString(),
            "java.lang.NullPointerException"
    ...
    fun verify_return_failure_datasource_if_token_invalid() = runTest {
        ...
        server.start()
        server.enqueue(
            MockResponse().setBody(tokenInvalidVisitante)
        )
        loadKoinModules(
            generateTestAppComponent(
                server.url("/").toString()
    ...
    fun verify_return_failure_repository_if_data_incorrect() = runTest {
        ...
        server.start()
        server.enqueue(
            MockResponse().setBody(resultVisitanteIncorrect)
        )
        loadKoinModules(
            generateTestAppComponent(
                server.url("/").toString()
        ...
        assertEquals(
            result.exceptionOrNull()!!.message,
            "IGetServerVisitante -> IVisitanteRepository.recoverAll"
        )
        assertEquals(
            result.exceptionOrNull()!!.cause.toString(),
            "java.lang.NullPointerException: Parameter specified as non-null is null: method br.com.usinasantafe.pcp.domain.entities.stable.Visitante.<init>, parameter cpfVisitante"
        )
    }
    ...
    fun verify_return_data_if_success_usecase() = runTest {
        ...
        assertEquals(
            resultVisitante.idVisitante,
            1
        )
        assertEquals(
            resultVisitante.nomeVisitante,
            "Visitante"
        )
        assertEquals(
            resultVisitante.cpfVisitante,
            "123.456.789-00"
        )
        assertEquals(
            resultVisitante.empresaVisitante,
            "Empresa Visitante"
        )
    }
