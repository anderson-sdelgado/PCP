...
class ICloseAllMovVisitTercTest : KoinTest {
    ...
    fun check_close_mov_open() = runTest {
        ...
        assertEquals(
            roomModelBefore1.statusMovEquipVisitTerc,
            StatusData.OPEN
        )
        val roomModelBefore2 =
            movEquipVisitTercDao.get(2)
        assertEquals(
            roomModelBefore2.statusMovEquipVisitTerc,
            StatusData.OPEN
        )
        val result = usecase()
        assertEquals(
        ...
        val roomModelAfter1 =
            movEquipVisitTercDao.get(1)
        assertEquals(
            roomModelAfter1.statusMovEquipVisitTerc,
            StatusData.CLOSE
        )
        val roomModelAfter2 =
            movEquipVisitTercDao.get(2)
        assertEquals(
            roomModelAfter2.statusMovEquipVisitTerc,
            StatusData.CLOSE
        )
    }
}