...
class ISendMovProprioTest : KoinTest {
    ...
    fun verify_return_true() = runTest {
        ...
        server.start()
        server.enqueue(
            MockResponse().setBody("""[{"idMovEquipProprio":1}]""")
        )
        loadKoinModules(
    ...
    fun verify_return_failure_datasource() = runTest {
        ...
        assertEquals(
            result.exceptionOrNull()!!.message,
            "ISendMovProprioList -> IMovEquipProprioRepository.send -> IMovEquipProprioRetrofitDatasource.send"
        )
        assertEquals(
            result.exceptionOrNull()!!.cause.toString(),
            NullPointerException().toString()
