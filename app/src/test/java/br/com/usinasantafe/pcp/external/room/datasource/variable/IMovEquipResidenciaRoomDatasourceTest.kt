...
class IMovEquipResidenciaRoomDatasourceTest {
    ...
    fun `Check return true if have mov open`() = runTest {
        ...
        assertEquals(
            model.motoristaMovEquipResidencia,
            "MOTORISTA TESTE"
        )
        assertEquals(
    ...
    fun `Check alter status in mov open set`() = runTest {
        ...
        assertEquals(
            roomModelBefore.statusMovEquipResidencia,
            StatusData.OPEN
        )
        ...
        assertEquals(
            roomModelAfter.statusMovEquipResidencia,
            StatusData.CLOSE
        )
    }
    ...
    fun `Check return roomModel if get execute correctly`() = runTest {
        ...
        assertEquals(
            roomModel.observMovEquipResidencia,
            "OBSERV TESTE"
        )
    }
    ...
    fun `Check return true if setMotorista execute correctly`() = runTest {
        ...
        val roomModelAfter = movEquipResidenciaDao.get(1)
        assertEquals(
            roomModelAfter.motoristaMovEquipResidencia,
            "MOTORISTA TESTE ALTERAR"
        )
        assertEquals(
    ...
    fun `Check return true if setOutside execute correctly null`() = runTest {
        ...
        val roomModelBefore = movEquipResidenciaDao.get(1)
        assertEquals(
            roomModelBefore.statusMovEquipForeignerResidencia,
        ...
        val roomModelAfter = movEquipResidenciaDao.get(1)
        assertEquals(
            roomModelAfter.statusMovEquipForeignerResidencia,
    ...
    fun `Check return true if setPlaca execute correctly`() = runTest {
        ...
        val datasource = IMovEquipResidenciaRoomDatasource(movEquipResidenciaDao)
        datasource.save(movEquipResidenciaRoomModel)
        val result = datasource.setPlaca(
            "PLACA TESTE ALTERAR",
            1
        )
        assertEquals(
        ...
        val roomModelAfter = movEquipResidenciaDao.get(1)
        assertEquals(
            roomModelAfter.placaMovEquipResidencia,
            "PLACA TESTE ALTERAR"
        )
        assertEquals(
            roomModelAfter.statusSendMovEquipResidencia,
    ...
    fun `Check return true if setVeiculo execute correctly`() = runTest {
        ...
        val roomModelAfter = movEquipResidenciaDao.get(1)
        assertEquals(
            roomModelAfter.veiculoMovEquipResidencia,
    ...
    fun `Check return list if have mov send`() = runTest {
        ...
        val entity = list[0]
        assertEquals(
            entity.dthrMovEquipResidencia,
            roomModel.dthrMovEquipResidencia
        )
        assertEquals(
    ...
    fun `setSent - Check return failure in setSent if not have mov`() = runTest {
        ...
        assertEquals(
            ...
            "IMovEquipResidenciaRoomDatasource.setSent"
        )
        assertEquals(
            result.exceptionOrNull()!!.cause.toString(),
            "java.lang.NullPointerException: Cannot invoke \"br.com.usinasantafe.pcp.infra.models.room.variable.MovEquipResidenciaRoomModel.setStatusSendMovEquipResidencia(br.com.usinasantafe.pcp.utils.StatusSend)\" because \"roomModel\" is null"
        )
    }
    ...
    fun `Check data returned in listSent`() =
        runTest {
            ...
            assertEquals(
                list[0].idMovEquipResidencia,
                1
            )
            assertEquals(
                list[1].idMovEquipResidencia,
                2
            )
        }
