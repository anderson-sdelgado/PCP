...
class ControleChaveEditListViewModelTest {
    ...
    private val viewModel = ControleChaveEditListViewModel(
        getMovChaveOpenList,
        closeAllMovChave
    )
    ...
    fun `recoverControleChaveOpenList - Check return failure if have error in GetMovChaveOpenList`() =
        runTest {
            ...
            assertEquals(
                state.failure,
                "ControleChaveEditListViewModel.recoverMovOpenList -> GetMovChaveOpenList -> java.lang.Exception"
            )
        }
    ...
    fun `closeAllMov - Check return failure if have error in CloseAllMovChave`() =
        runTest {
            ...
            assertEquals(
                state.failure,
                "ControleChaveEditListViewModel.closeAllMov -> CloseAllMovChave -> java.lang.Exception"
            )
        }
